const fs = /*glsl*/ `#version 300 es
	...
	void main(){
		...
		vec2 v = fold(uv, 150., _div, rot_r, _drift, _sdf); //-n
		...
		vec2 uv_shift = mix(tuv,v, mlev*mix(max(1.-_oscmixm,0.),chr,select_lev));
		...
		float idx_r = floor(hash11(6.*(3.3+idx+idx2))*11.);
		vec2 offsr = offset(offs+offs_fine, vec2(10., 7.));
		vec3 ct1_r = texture(u_sampler, vec3(uv_shift, idx_r)+vec3(offsr,0.)).xyz;
